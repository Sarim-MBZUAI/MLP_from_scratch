...
class BatchNorm1d:
    ...
    def backward(self, dLdBZ):
        ...
        dLdM = np.sum(dLdNZ * -1 / np.sqrt(self.V + self.eps), axis=0, keepdims=True) + dLdV * np.sum(-2 * (self.Z - self.M), axis=0) / N  # TODO

        dLdZ = (dLdNZ / np.sqrt(self.V + self.eps)) + (dLdV * 2 * (self.Z - self.M) / N) + (dLdM / N)  # TODO
